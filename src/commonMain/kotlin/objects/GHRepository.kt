...
@Serializable
class GHRepository internal constructor(
    /**
    ...
    @SerialName("allow_rebase_merge") val rebaseMergeAllowed: Boolean
) : GitHubObject() {
    suspend fun fetchCommits(): GHResult<List<GHCommit>> = getWithConfig(commitsUrl.removeCurlyBraces())

    suspend fun fetchIssues(): GHResult<List<GHIssue>> = getWithConfig(issuesUrl.removeCurlyBraces())
     ...
     */
    suspend fun delete(): GHResult<Unit> = root.deleteRepository(owner = owner.login, repository = name)
     ...
     * - [HttpStatusCode.UnprocessableEntity]: Validation failed or the endpoint has been spammed.
     */
    suspend fun createTree(baseTreeSha: String, treeItems: List<GHTree.Item>): GHResult<GHTree> {
        return root.createTree(owner.login, name, baseTreeSha, treeItems)
    }

    suspend fun createTree(baseTree: GHBranch, treeItems: List<GHTree.Item>): GHResult<GHTree> {
        return createTree(baseTree.sha1, treeItems)
    }

    suspend fun fetchBranch(branchName: String): GHResult<GHBranch> = root.fetchBranch(owner.login, name, branchName)

    suspend fun createCommit(commitMessage: String, tree: GHTree): GHResult<GHCommitResponse> {
        return root.createCommit(owner.login, name, commitMessage, tree)
    }

    suspend fun fetchReference(reference: String): GHResult<GHRef> = root.fetchReference(owner.login, name, reference)

    suspend fun deleteReference(reference: String): GHResult<Unit> = root.deleteReference(owner.login, name, reference)
