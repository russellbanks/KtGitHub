...
import objects.GHBranch
import objects.GHCommitResponse
import objects.GHContent
...
class GitHub @OptIn(ExperimentalSerializationApi::class) internal constructor(
     ...
     */
    suspend fun fetchRepository(owner: String, repository: String): GHResult<GHRepository> {
     ...
     * - [HttpStatusCode.NotFound]: The repository does not exist.
     */
    suspend fun deleteRepository(owner: String, repository: String): GHResult<Unit> {
    ...
    suspend fun fetchDirectoryContent(owner: String, repository: String, path: String): GHResult<List<GHContent>> {
     ...
     */
    suspend fun fetchUser(username: String): GHResult<GHUser> = getWithConfig("$apiUrl/users/$username")
     ...
     */
    suspend fun fetchAllEmojis(): GHResult<Map<String, String>> = getWithConfig("$apiUrl/emojis")
     ...
     */
    suspend fun fetchIssue(owner: String, repository: String, issueNumber: Int): GHResult<GHIssue> {
     ...
     */
    suspend fun fetchIssueAssignees(owner: String, repository: String): GHResult<List<GHUser>> {
     ...
     */
    suspend fun fetchCommonlyUsedLicenses(
     ...
     */
    suspend fun fetchLicense(license: String): GHResult<GHLicense> = getWithConfig("$apiUrl/licenses/$license")
     ...
     * if the request is successful.
     */
    suspend fun fetchLicenseForRepository(owner: String, repository: String): GHResult<GHLicense> {
    ...
    suspend fun fetchBranch(owner: String, repository: String, branchName: String): GHResult<GHBranch> {
     ...
     * - [HttpStatusCode.NotFound]: The resource is not found.
     */
    suspend fun fetchReference(owner: String, repository: String, reference: String): GHResult<GHRef> {
        return getWithConfig("$apiUrl/repos/$owner/$repository/git/refs/$reference")
    }

    suspend fun fetchMatchingReferences(owner: String, repository: String, reference: String): GHResult<List<GHRef>> {
    ...
    suspend fun deleteReference(owner: String, repository: String, ref: String): GHResult<Unit> {
     ...
     */
    suspend fun fetchAllGitIgnoreTemplates(): GHResult<List<String>> {
     ...
     */
    suspend fun fetchGitIgnoreTemplate(name: String): GHResult<GHGitIgnoreTemplate> {
        return getWithConfig("$apiUrl/gitignore/templates/$name")
    }

    suspend fun renderMarkdownDocument(text: String): GHResult<String> = postWithConfig("$apiUrl/markdown") {
        @Serializable
    ...
    companion object {
        ...
        fun new(init: GitHubBuilder.() -> Unit): GitHub = GitHubBuilder().apply(init).build()
    }
}

val GHJson get() = ContentType("application", "vnd.github+json")
